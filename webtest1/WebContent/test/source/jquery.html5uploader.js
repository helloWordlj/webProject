...
$.fn.html5uploader = function(opts){
	...
	this.each(function(){
		var _this = $(this);
		//先添加上file按钮和上传列表
		var inputstr = '<input class="uploadfile" style="visibility:hidden;" type="file" name="files"';
		if(option.multi){
			inputstr += 'multiple';
		} 
		inputstr += '/>';
		...
		var ZXXFILE = {
			...
			filter : function(files) { // 选择文件组的过滤方法
				...
				if (!typeArray) {
					...
					for ( var i in files) {
						if (files[i].constructor == File) {
							var fileType=files[i].name.lastIndexOf(".");
							if(fileType>0){
								fileType=files[i].name.substring(fileType+1);
