var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	renderRow: function(target, fields, frozen, rowIndex, rowData){
		...
		if (frozen && opts.rownumbers){
			...
			//----------------------以下是增加的内容---------------------  
			var cell_width = 25;  
			if((rownumber+"").length > 3 && (rownumber+"").length <= 4){
                cell_width = 30;
            }else if((rownumber+"").length > 4 && (rownumber+"").length < 7){
			cell_width = 35;
			}else if((rownumber+"").length > 6 && (rownumber+"").length < 9){  
			cell_width = 50;  
			}else if((rownumber+"").length > 8 && (rownumber+"").length < 11){  
			cell_width = 65;  
			}  
			cc.push("<td class=\"datagrid-td-rownumber\"><div style='width:"+cell_width+"px' class=\"datagrid-cell-rownumber\">"+rownumber+"</div></td>");  
