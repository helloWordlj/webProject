...
public class HttpClient implements Serializable {
	...
	/**
	 * Move temporarily:The requested resource temporary from different URI in response to a request.
	 */
	private static final int NOT_RESOURCE = 302;
	/**
	 * Not Modified: If the client sends a GET request with the condition and the request is allowed, and the content of
	 * the document (since the last visit or request according to the conditions of) did not change, the server should
	 * return a status code.
	 */
	private static final int NOT_MODIFIED = 304;
	/**
	 * Bad Request: Semantic error, the current request cannot be understood by the server.
	 */
	...
	/**
	 * Not Acceptable:The content characteristics of the requested resource can not meet the head condition request,
	 * thus unable to generate the response entity.
	 */
	private static final int NOT_ACCEPTABLE = 406;
	/**
	 * Internal Server Error:The server encountered an unexpected condition, cause it can't fulfill the request
	 * processing.
	 */
	private static final int INTERNAL_SERVER_ERROR = 500;
	/**
	 * Bad Gateway: As a gateway or proxy work server attempts to execute the request, response from the upstream server
	 * received an invalid.
	 */
	private static final int BAD_GATEWAY = 502;
	/**
	 * Service Unavailable:Due to a temporary server maintenance or overload, the server cannot process the request.
	 */
	...
	public HttpResponse post(String url, String value, String contentType, String charset, String headerName, String headerValue)
			throws SystemException {
		log("Request:");
		log("POST:" + url);
		PostMethod postMethod = new PostMethod(url);
	 ...
	 * @throws SystemException
	 */
	public HttpResponse httpRequest(HttpMethod method) throws SystemException {
		int responseCode = -1;
